/*During the execution of trident we store 6 differente structure, in order to identify all the structure we use a label in the memory we store the label before all storing in the flash
->logRecordstats : 1
->logRecordRx : 2 
->logRecordRxError : 3
->logRecordTx : 4
...
static void ReadMemory(uint8_t dataSise)
{
  ...
  while(readedBytes < dataSize-8 )//8 because its the size of the smallest structure
  {
